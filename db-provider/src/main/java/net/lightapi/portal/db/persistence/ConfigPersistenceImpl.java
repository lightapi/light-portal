...
public class ConfigPersistenceImpl implements ConfigPersistence {
    ...
    */
        ...
        String sql =
                """
                      WITH active_instance AS (
                           SELECT * FROM instance_t WHERE active = true
                           ),
                           active_product_version_config AS (
                           SELECT * FROM product_version_config_t WHERE active = true
                           ),
                           active_product_version_config_property AS (
                           SELECT * FROM product_version_config_property_t WHERE active = true
                           ),
                           active_config_property AS (
                           SELECT * FROM config_property_t WHERE active = true
                           ),
                           active_product_version AS (
                           SELECT * FROM product_version_t WHERE active = true
                           ),
                           active_product_property AS (
                           SELECT * FROM product_property_t WHERE active = true
                           ),
                           active_product_version_property AS (
                           SELECT * FROM product_version_property_t WHERE active = true
                           ),
                           active_environment_property AS (
                           SELECT * FROM environment_property_t WHERE active = true
                           ),
                           active_config AS (
                           SELECT * FROM config_t WHERE active = true
                           ),
                              applicable_instance_config_properties AS (
                                ...
                                JOIN active_product_version_config pvc
                                ON i.host_id = pvc.host_id
                                AND i.product_version_id = pvc.product_version_id
                                JOIN active_product_version_config_property pvcp
                                ON i.host_id = pvcp.host_id
                                AND i.product_version_id = pvcp.product_version_id
                                WHERE i.host_id = ? AND i.instance_id = ?
                                ),
                                   property_values AS (
                                      SELECT cp.property_id,
                                   ...
                                   SELECT COUNT(*) OVER () AS total,
                                        ...
                                        JOIN applicable_instance_config_properties aicp
                                        ON c.config_id = aicp.config_id
                                        AND cp.config_id = aicp.config_id
                                        AND cp.property_id = aicp.property_id
    ...
    public Result<String> getApplicableConfigPropertiesForInstanceApi(
        ...
        String sql =
            """
               WITH active_instance_api AS (
                    SELECT * FROM instance_api_t WHERE active = true
                    ),
                    active_instance AS (
                    SELECT * FROM instance_t WHERE active = true
                    ),
                    ...
                    FROM active_instance_api ia
                         JOIN active_instance i
                         ON ia.instance_id = i.instance_id
                         AND ia.host_id = i.host_id
                         JOIN active_product_version_config pvc
                         ON i.host_id = pvc.host_id
                         AND i.product_version_id = pvc.product_version_id
                         JOIN active_product_version_config_property pvcp
                    ...
                    property_values AS (
                      ...
                      SELECT COUNT(*) OVER () AS total,aicp.host_id, aicp.instance_id,
                             ...
                             FROM active_config c
                             JOIN active_config_property cp ON c.config_id = cp.config_id
                             JOIN applicable_instance_config_properties aicp\s
                             ON c.config_id = aicp.config_id\s
                             AND cp.config_id = aicp.config_id\s
                             AND cp.property_id = aicp.property_id
                             LEFT JOIN property_values pv ON cp.property_id = pv.property_id
    ...
    public Result<String> getApplicableConfigPropertiesForInstanceApp(
        ...
        String sql =
            """
               WITH active_instance_app AS (
                    SELECT * FROM instance_app_t WHERE active = true
                    ),
                    active_instance AS (
                    SELECT * FROM instance_t WHERE active = true
                    ),
    ...
    public Result<String> getApplicableConfigPropertiesForInstanceAppApi(
        ...
        String sql =
            """
               WITH active_instance_app_api AS (
                    SELECT * FROM instance_app_api_t WHERE active = true
                    ),
                    active_instance_app AS (
                    SELECT * FROM instance_app_t WHERE active = true
                    ),
                    active_instance_api AS (
                    SELECT * FROM instance_api_t WHERE active = true
                    ),
                    active_instance AS (
                    SELECT * FROM instance_t WHERE active = true
                    ...
                    SELECT DISTINCT pvc.config_id, pvcp.property_id, i.host_id, i.instance_id, i.product_version_id
                           ...
                           JOIN active_instance_app iapp
                           ON iappapi.instance_app_id = iapp.instance_app_id
                           AND iappapi.host_id = iapp.host_id
                           JOIN active_instance_api iapi
                           ON iappapi.instance_api_id = iapi.instance_api_id
                           AND iappapi.host_id = iapi.host_id
                               JOIN active_instance i
                               ON iapp.instance_id = i.instance_id
                               AND iapp.host_id = i.host_id
                               AND iapi.instance_id = i.instance_id
                               AND iapi.host_id = i.host_id
                               JOIN active_product_version_config pvc
                               ON i.host_id = pvc.host_id
                               AND i.product_version_id = pvc.product_version_id
                               JOIN active_product_version_config_property pvcp
    ...
    public Result<String> getPromotableInstanceConfigs(String hostId, String instanceId, Set<String> propertyNames, Set<String> apiUids) {
        ...
        final String instanceConfigsSql = """
                WITH active_instance_property AS (
                     SELECT * FROM instance_property_t WHERE active = true
                     ),
                     active_config_property AS (
                     SELECT * FROM config_property_t WHERE active = true
                     ),
                     ...
                     SELECT
                       ...
                       CASE
                           ...
                           FROM active_instance_property as ip
                                ...
                                JOIN active_instance as instance ON instance.instance_id = ip.instance_id
        ...
        final String subresourceConfigsSql = """
                    WITH active_instance_api AS (
                         ...
                         active_api_version AS (
                         SELECT * FROM api_version_t WHERE active = true
                         ),
                         active_instance AS (
                         SELECT * FROM instance_t WHERE active = true
                         ),
                         active_instance_api_path_prefix AS (
                         SELECT * FROM instance_api_path_prefix_t WHERE active = true
                         ),
                         active_config_property AS (
                         SELECT * FROM config_property_t WHERE active = true
                         ),
                         ...
                         active_instance_api_property AS (
                         SELECT * FROM instance_api_property_t WHERE active = true
                         ),
                         active_instance_app_property AS (
                         SELECT * FROM instance_app_property_t WHERE active = true
                         ),
                         active_instance_app AS (
                         SELECT * FROM instance_app_t WHERE active = true
                         ),
                         active_instance_app_api_property AS (
                         SELECT * FROM instance_app_api_property_t WHERE active = true
                         ),
                         active_instance_app_api AS (
                         SELECT * FROM instance_app_api_t WHERE active = true
                         ),
                         params AS MATERIALIZED (
                         SELECT CAST(v.host_id AS UUID) AS host_id,
                                CAST(v.instance_id AS UUID) AS instance_id,
                                     ...
                                     instance_api_path_prefix AS ( 
                                        SELECT STRING_AGG(iapp.path_prefix, ', ' ORDER BY iapp.path_prefix) AS api_path_prefixes,
                                               ...
                                               FROM active_instance_api ia
                                                    ...
                                                    JOIN active_instance i
                                                    ON i.instance_id = ia.instance_i
                                                    AND i.host_id = ia.host_id
                                                    JOIN params p
                                                    ...
                                                    LEFT JOIN active_instance_api_path_prefix iapp
                                                    ON ia.instance_api_id = iapp.instance_api_id
                                                    AND ia.host_id = iapp.host_id
        ...
        final String instanceCustomFilesSql = """
                    WITH active_instance_file AS (
                         SELECT * FROM instance_file_t WHERE active = true
                    ),
                    active_instance AS (
                      SELECT * FROM instance_t WHERE active = true
                    ),
                    params AS MATERIALIZED (
                          ...
                          SELECT f.host_id, f.instance_id, f.instance_file_id, f.file_name, f.file_value, f.file_type
                                 ...
                                 JOIN active_instance i
                                 ON f.instance_id = i.instance_id
                                 AND f.host_id = i.host_id
                                 JOIN params p
                                 ON f.host_id = p.host_id
                                 AND f.instance_id = p.instance_id
